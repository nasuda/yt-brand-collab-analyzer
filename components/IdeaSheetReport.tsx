...
export const IdeaSheetReport = forwardRef<HTMLDivElement, IdeaSheetProps>(
  function IdeaSheetReport({ brandName, channelName, idea }, ref) {
    ...
    return (
      ...
      >
        ...
        <div data-section="header" style={{ padding: "15mm 20mm 10mm" }}>
          ...
          >
            ※本シートはあくまで参考企画案です。最終的な企画はクリエイター様の判断にお任せします。
          </div>
        </div>
        ...
        {(pi.brandMustDo?.length > 0 || pi.brandMustNot?.length > 0) && (
          <div data-section="requirements" style={{ padding: "8mm 20mm" }}>
            <h2 style={sectionTitle}>この企画固有の要件</h2>
            <p style={{ ...valueStyle, color: "#6b7280", marginBottom: "10px" }}>
              ※共通ルール（メインブリーフ記載）に加えて、この企画に適用される追加要件です。
            </p>
        ...
        {pi.sampleOpening && (
          <div data-section="opening" style={{ padding: "8mm 20mm" }}>
            <div style={labelStyle}>導入セリフ例（参考）</div>
            <div
              style={{
                padding: "10px 14px",
                background: "#f9fafb",
        ...
        <div style={{ padding: "8mm 20mm", borderTop: "1px solid #e5e7eb", marginTop: "4mm" }}>
          <div style={{ fontSize: "9px", color: "#9ca3af", textAlign: "center" }}>
            本シートは {channelName} 様向けに作成された参考企画案です。
            企画の採用・変更はクリエイター様のご判断にお任せいたします。
          </div>
        </div>
      </div>
    );
