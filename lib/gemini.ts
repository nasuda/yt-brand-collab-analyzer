      ...
      items: {
        ...
        properties: {
          ...
          postingInstruction: {
            type: Type.OBJECT,
            properties: {
              contentDirection: { type: Type.STRING, description: "構成フレームワーク（固定パートと自由パートを明示した構成ガイド）" },
              ...
              brandMustDo: {
                type: Type.ARRAY,
                items: { type: Type.STRING },
                description: "ブランドとして必ず守ってほしい要件（法的義務、必須訴求ポイント、商品名の正式表記等）。3-5個",
              },
              brandMustNot: {
                type: Type.ARRAY,
                items: { type: Type.STRING },
                description: "絶対にやってはいけないNG行為・表現（競合言及、過度な効果訴求、特定ワード等）。2-4個",
              },
              creatorFreedom: {
                type: Type.ARRAY,
                items: { type: Type.STRING },
...
##### ブランド必須要件（brandMustDo）
法的義務（PR表記、景表法対応）、商品の正式名称、必ず伝えるべき訴求ポイント等。
マーケターがチェックリストとして使えるよう、具体的かつ簡潔に。

##### NG行為・表現（brandMustNot）
競合ブランドの言及、過度な効果訴求、ブランドイメージを損なう表現等。
「なぜNGか」の理由は不要、事実だけを簡潔に列挙する。
...
##### 概要欄（descriptionBoxSuggestion）
PR表記の位置、ブランドリンク、ハッシュタグ、クーポンコード、免責事項を含む概要欄テンプレート。
