...
**プリ分析データ（signatureElements, contentTypes, engagementDrivers）を参照し、そのクリエイター固有の強みに即して書く。**
...
**signatureElements（キャッチフレーズ、定番の挨拶）を直接反映する。**
動画タイトルの口調や、コメント欄でファンが使う呼び方なども参考に。
...
export async function analyzeBrandFit(
  ...
  if (!Array.isArray(raw.collabIdeas) || raw.collabIdeas.length === 0) {
    ...
    raw.collabIdeas = raw.collabIdeas.filter((idea: unknown) => {
      ...
      if (!Array.isArray(pi.brandMustDo)) return false;
      if (!Array.isArray(pi.brandMustNot)) return false;
      if (!Array.isArray(pi.creatorFreedom)) return false;
