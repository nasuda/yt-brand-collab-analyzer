...
const responseSchema = {
  ...
  properties: {
      ...
      items: {
        ...
        properties: {
          ...
          postingInstruction: {
            type: Type.OBJECT,
            properties: {
              contentDirection: { type: Type.STRING, description: "構成フレームワーク（固定パートと自由パートを明示。「S1」「S2」等の抽象IDは使わず、各パートの意図を自然言語で記述すること）" },
              descriptionBoxSuggestion: { type: Type.STRING, description: "概要欄の完全テンプレート（PR表記→動画説明→ブランドリンク(UTM付きプレースホルダー)→クーポンコード枠→公式ハッシュタグ→免責事項→クリエイター通常セクション の構造で出力。プレースホルダーは[角括弧]で）" },
              keyMessages: {
                type: Type.ARRAY,
                items: { type: Type.STRING },
                description: "伝えるべきキーメッセージ（3〜5個）",
              },
              toneAndManner: { type: Type.STRING, description: "企画全体の雰囲気の指針。クリエイターの普段のトーンとブランドトーンの接点" },
              brandMustDo: {
                type: Type.ARRAY,
                items: { type: Type.STRING },
                description: "この企画固有のブランド必須要件（campaignRulesに含まれるキャンペーン共通ルールは除外すること）。この企画にだけ必要な訴求ポイント・手順等。1-3個",
              },
              brandMustNot: {
                type: Type.ARRAY,
                items: { type: Type.STRING },
                description: "この企画固有のNG行為・表現（campaignRulesのuniversalMustNotに含まれるキャンペーン共通NGは除外すること）。この企画特有のリスクに対するNG事項。0-2個",
              },
              creatorFreedom: {
                type: Type.ARRAY,
                items: { type: Type.STRING },
                description: "クリエイターの自由裁量に委ねる領域。クリエイターの得意パターンやスタイル要素を踏まえ、具体的にどこを任せるかを示す。3-5個",
              },
              creatorContext: { type: Type.STRING, description: "この企画がこのクリエイターに合う理由。具体的な動画タイトル・コメント傾向・コンテンツパターンを参照し、クリエイターの強みとの接続を示す（2-3文）" },
              sampleOpening: { type: Type.STRING, description: "このクリエイターが実際に言いそうな動画冒頭の導入セリフ例。クリエイターのキャッチフレーズ・定番の挨拶・口調を反映（30-50字）" },
            },
            required: ["contentDirection", "descriptionBoxSuggestion", "keyMessages", "toneAndManner", "brandMustDo", "brandMustNot", "creatorFreedom", "creatorContext", "sampleOpening"],
          },
    ...
    campaignOverview: {
      type: Type.OBJECT,
      properties: {
        objective: { type: Type.STRING, description: "施策の目的。マーケ用語ではなく、生活者にもたらしたい変化を感情的に描写（1-2文）" },
        challenge: { type: Type.STRING, description: "課題感・背景。市場データではなく生活者の実感に根ざした課題意識（1-2文）" },
        insight: { type: Type.STRING, description: "ターゲットインサイト。生活者のリアルな感情・葛藤・欲求を広告コピーのような力強い表現で（1-2文）" },
        targetAudience: { type: Type.STRING, description: "ターゲット像。属性+ライフスタイル+心理状態を含めて描写（1文）" },
      },
      required: ["objective", "challenge", "insight", "targetAudience"],
      description: "施策概要（brandDescriptionから抽出。クリエイターが読んで「参加したい」と感じるエモーショナルな文体で）",
    },
    campaignRules: {
      type: Type.OBJECT,
      properties: {
        universalMustDo: {
          type: Type.ARRAY,
          items: { type: Type.STRING },
          description: "全企画に共通して適用される必須要件（PR表記、景表法対応、薬機法対応、法的義務、商品名の正式表記等）。3-5個",
        },
        universalMustNot: {
          type: Type.ARRAY,
          items: { type: Type.STRING },
...
### 呼称ルール（最優先・全出力フィールドに適用）
このブリーフはクリエイター本人が読む文書です。以下を厳守してください:
- 「あなた」「あなたの」「彼」「彼女」「彼の」「彼女の」など、クリエイターを指す代名詞は**一切使用禁止**。
- 必ずクリエイター名（チャンネル名）+「さん」で呼ぶこと（例: 「NIKO LIFEさんの世界観」「○○さんの審美眼」）。
- 名前の繰り返しが多くなる場合は、主語を省略するか文を再構成して自然な日本語にすること。
- この規則は brandAlignmentReasoning, contentStyleSummary, strengths, strategicNarrative, creatorWorldview, avoidanceNote, inspirationSeeds, creatorContext, creatorFreedom を含む**全ての出力フィールド**に適用する。
...
- **クリエイターの職歴・学歴・資格・経歴など、動画データ・コメント・チャンネル概要から直接確認できない情報を断定的に記載しない。** 動画の内容から推測できる知識・スキルに言及する場合は「動画から〜に精通していることが窺える」のように根拠と推定表現を明示すること。
...
- リスクレベル分散: 安全策1案（最大） + 標準2〜3案 + 挑戦的な企画2案以上（必須）
...
**企画案はあくまで参考案であり、最終的な企画はクリエイターにお任せする**というスタンスで書く。
クリエイターの世界観・判断を尊重しつつ、ブランドとして最低限守ってほしいことだけを明確にする。

#### 企画のクリエイティブ強度（必須）

**「普通にいい企画」は不合格。** 視聴者のフィードで目を止めるには「えっ？」と思わせる要素が必要。

各企画を以下の「クリエイティブ強度チェック」で自己検証し、合格する企画のみ出力すること:
1. **予想外性テスト**: この企画を聞いて「え、マジで？」と思うか？ 予想通りの展開しかなければ失格。
2. **SNSシェアテスト**: 友人にLINEで「これ見てww」と送りたくなるか？ 説明が必要なら失格。
3. **コメント誘発テスト**: 視聴者が「自分も○○したい」「結果が気になる」とコメントせずにいられないか？
4. **クリエイター本気テスト**: 案件でなくてもこのクリエイターが自腹でやりたがるか？

クリエイティブの引き上げ方:
- **Core Insight（campaignOverview.insight）を企画の中心に据える**: インサイトを「伝える」のではなく「体験させる」企画にする
- **スケールの飛躍**: ブランドの支援があるからこそ実現する、普段の動画では不可能なスケール感を入れる
- **対立構造・賭け・制約**: 「○○ vs ○○」「○○だけで○日生活」「○○を一切使わずに○○」等の制約や対決要素を仕込む
- **視聴者参加の設計**: コメント投票で展開が変わる、視聴者のお題に挑戦する等、受動的視聴を能動的参加に変える仕掛け
- **意外な組み合わせ**: ブランドの要素をクリエイターの世界観と「最も遠い場所」で掛け合わせる
...
- 以下のような「テンプレ企画」は禁止（安直すぎる）:
  - 「○○を実際に使ってレビューしてみた」（単純レビュー）
  - 「○○のある1日ルーティン」（ブランド要素を差し替えるだけの汎用ルーティン）
  - 「○○を○日間使い続けた結果…」（変化が予測可能なビフォーアフター）
  これらを出す場合は、必ず独自のツイスト（制約、対決、視聴者参加等）を加えること。
...
投稿指示書は「台本」ではなく「コラボレーションガイド」として書く。
クリエイターが読んで「自分のチャンネルでやりたい」と思える内容にすること。

##### キャンペーン共通ルール vs 企画固有ルール（必須分離）
brandMustDo / brandMustNot の項目を以下のように分離すること:
- **campaignRules.universalMustDo**: 全企画に共通する法的義務・ブランドガイドライン
  （例: 「#PR表記を動画冒頭5秒以内と概要欄冒頭に表示」「景品表示法に基づく表記」「商品名の正式表記: ○○」）
- **campaignRules.universalMustNot**: 全企画に共通するNG事項
  （例: 「競合ブランドへの言及禁止」「効果効能の断定表現禁止」「特定ワードの使用禁止」）
- **各企画のpostingInstruction.brandMustDo**: その企画にだけ適用される固有の要件
  （例: 「検証動画では必ず計測数値を画面表示する」「料理動画では完成品を3秒以上映す」）
- **各企画のpostingInstruction.brandMustNot**: その企画にだけ適用される固有のNG
  （例: 「比較企画で他社製品を貶める表現は禁止」）
campaignRulesに記載した項目を各企画のbrandMustDo/brandMustNotで繰り返すことは禁止。

##### 構成ガイド（contentDirection）
スクリプトではなく「構成フレームワーク」として書く。
**最重要: 構成の「流れ」自体をクリエイターの型（bestPerformingType / contentTypes）から逆算すること。**
「導入→展開→ブランドモーメント→締め」のような全クリエイター共通の汎用テンプレートは禁止。
クリエイターの得意な展開パターンにブランド要素を自然に組み込んだ、そのクリエイター固有の構成を設計する。

##### 構成ガイドの記述ルール（厳守）
- 「S1」「S2」「シーン1」「パート1」等の抽象的なIDや番号で構成を参照してはいけない。
- 各パートは意図・内容を自然言語で説明すること。
  NG例: 「S1で商品紹介を行い、S2で施設外観を映す」
  OK例: 「施設到着後、外観と内装の魅力が伝わるように撮影し、広々としたリビングやキッチンなどの空間を紹介する」
- 構成の各ステップには「このパートの目的」と「クリエイターがやること」を書く。

構成設計の思考プロセス:
1. クリエイターの bestPerformingType を確認（検証系? ルーティン系? ランキング系? Vlog系?）
2. その型の典型的な展開パターンを再現する（検証なら「仮説→実験→結果発表」、ルーティンなら「朝→昼→夜の時間軸」等）
3. そのパターンの中にブランドの訴求ポイントが自然に入る場所を特定する
4. [固定]（ブランドが求める部分）と[自由]（クリエイターに委ねる部分）を明示する

型別の構成例（これを参考に、実際のクリエイターの型に合わせて設計する）:
- 検証系: 「仮説提示: [自由] → 準備・条件設定: [自由] → 検証実行: [自由+固定] ブランド要素を検証対象に組み込む → 結果発表・考察: [固定] ブランドの価値を結果として示す → 視聴者への問いかけ: [自由]」
- ルーティン系: 「朝の支度: [自由] → 日中の活動: [自由+固定] ブランド要素が日常に溶け込む場面 → 夕方〜夜: [自由] → 振り返り: [固定] ブランドがもたらした変化」
- ランキング・比較系: 「ランキング導入: [自由] → 各項目紹介: [自由+固定] ブランド要素を比較軸に含む → 結論・1位発表: [固定] ブランドの強みが際立つ結論 → コメント募集: [自由]」
- トーク・語り系: 「テーマ提示: [自由] → 体験談・エピソード: [自由+固定] ブランド体験を自分の言葉で語る → 深掘り・持論展開: [自由] → まとめ: [固定] ブランドの本質的価値」
- Vlog系: 「出発・移動: [自由] → 到着・第一印象: [自由] → 体験ハイライト: [自由+固定] ブランドならではの瞬間 → 翌日・帰路: [自由+固定] 体験後の変化・感想」
上記はあくまで参考パターン。実際のクリエイターの動画構成を分析し、その人固有の「いつもの流れ」に最も近い構成にすること。

##### ブランド必須要件（brandMustDo）— 企画固有のみ
campaignRules.universalMustDoに含まれる共通ルール（PR表記、法的義務等）は**絶対に繰り返さない**。
この企画にだけ必要な固有の訴求ポイント・手順・表現ルールを記載する。
例: 「この検証企画では数値結果を画面上にテロップで表示する」「BBQシーンでは食品の安全な取り扱いを示す」

##### NG行為・表現（brandMustNot）— 企画固有のみ
campaignRules.universalMustNotに含まれる共通NG（競合言及禁止等）は**絶対に繰り返さない**。
この企画特有のリスクに対するNG事項のみ記載する。共通NGで十分な場合は空配列でもよい。

##### クリエイターの自由裁量（creatorFreedom）
クリエイターに「ここはお任せします」と伝える領域。
**<PRE_ANALYSIS>がある場合**: プリ分析データ（signatureElements, contentTypes, engagementDrivers）を参照し、そのクリエイター固有の強みに即して書く。
汎用的な「編集は自由」ではなく、「〇〇（シグネチャー要素）をそのまま活かしてOK」のように具体化する。例:
- 「導入はいつもの『○○○』の挨拶からでOK」（← signatureElementsを参照）
- 「検証系の展開（○○動画のような流れ）はそのまま活かして」（← bestPerformingTypeを参照）
- 「テロップ・BGM・カットはチャンネルの通常スタイルで」
- 「視聴者が反応する○○要素（← engagementDrivers）を活かしたアレンジ歓迎」
**<PRE_ANALYSIS>がない場合**: 動画データ（タイトル、タグ、概要）から読み取れるクリエイターの特徴に基づいて書く。動画タイトルの傾向やフォーマットから推測できる得意パターンを参照すること。

##### クリエイターコンテキスト（creatorContext）
**この企画がこのクリエイターに合う理由を、具体的なデータを参照して書く。**
**<PRE_ANALYSIS>がある場合**:
- 具体的な動画タイトルを参照する（例:「『○○してみた』（再生数○万回）のような検証フォーマットをベースに…」）
- コメント分析結果を参照する（例:「コメントで『○○もやって！』というリクエストが多いので…」）
- クリエイターの得意パターンとブランドの接続点を示す
**<PRE_ANALYSIS>がない場合**: 動画データ（タイトル、再生数、タグ）から読み取れる情報に基づいて、クリエイターの強みとブランドの接続点を示す。
クリエイターに「自分のチャンネルをちゃんと見てくれている」という信頼感を与え、コラボ参加意欲を高める。

##### 導入セリフ例（sampleOpening）
そのクリエイターが実際に動画冒頭で言いそうなセリフ。
**<PRE_ANALYSIS>がある場合**: signatureElements（キャッチフレーズ、定番の挨拶）を直接反映する。
**<PRE_ANALYSIS>がない場合**: 動画タイトルの口調やチャンネル名から推測される話し方で書く。
コメント欄でファンが使う呼び方なども参考に。

##### キーメッセージ（keyMessages）
ブランドの訴求ポイントを、クリエイターが自分の言葉で言い換えた形で書く。
台本ではなく「こういうニュアンスで伝えてほしい」という参考例。

##### トーン&マナー（toneAndManner）
企画全体の雰囲気の指針。クリエイターの普段のトーンとブランドトーンの接点を示す。

##### 概要欄テンプレート（descriptionBoxSuggestion）
概要欄は以下の構造を持つ完全なテンプレートとして出力すること。
プレースホルダーには [角括弧] を使い、クリエイターが埋められるようにする。

必須セクション（この順序で）:
1. **PR表記**（1行目、必須）: 「提供: [ブランド名]」
2. **動画の一行説明**: この動画の内容を1-2文で（クリエイターが書き換えてよい部分）
3. **ブランド公式リンク**:
   - 公式サイト: [URL?utm_source=youtube&utm_medium=creator&utm_campaign=企画名]
   - キャンペーンLP: [URL]（該当する場合）
4. **クーポン・特典**（該当する場合）:
   - クーポンコード: [CREATOR_CODE]
   - 特典内容: [割引内容・期間]
5. **公式ハッシュタグ一覧**: #PR #[ブランド名] #[キャンペーンハッシュタグ] + クリエイター任意のタグ
6. **免責事項・法的表記**:
   - 「※本動画は[ブランド名]とのタイアップです」
   - 商品カテゴリに応じた免責（化粧品→「※効果には個人差があります」、食品→「※個人の感想です」等）
   - 「※動画内の情報は[撮影日]時点のものです」
7. **クリエイター通常セクション**: [クリエイターの通常の概要欄フォーマット（SNSリンク等）をここに]

テンプレートは改行を適切に使い、視覚的に見やすくすること。
実際のURLやクーポンコードは不明なのでプレースホルダーを使う。
...
brandDescription から施策の核となる情報を抽出し、クリエイターブリーフの冒頭に表示する。
クリエイターが読んで「この施策に参加したい」と感じる、エモーショナルで共感を呼ぶ文体で書くこと。

- objective: ブランドが達成したいゴール。ただし「認知拡大」のようなマーケティング用語だけでなく、
  その先にある生活者の変化まで書く。
  NG: 「新規ユーザー獲得を目的とした認知施策」
  OK: 「"知らなかった自分を後悔する"——そんな出会いを1人でも多くの人に届ける」

- challenge: この施策が必要な背景。市場データだけでなく、生活者の実感に根ざした課題意識で書く。
  NG: 「市場競争が激化し、ブランド認知率の向上が課題」
  OK: 「選択肢が多すぎて"とりあえず有名なやつ"で済ませてしまう人が多い。本当に合うものに出会えていない」

- insight: ターゲットの心理・行動の核心。**最も重要なフィールド。**
  クリエイターが「わかる！」と膝を打つような、生活者のリアルな感情・葛藤・欲求を描写する。
  広告コピーのような力強い表現を使ってよい。
  NG: 「ターゲット層は品質と価格のバランスを重視する傾向がある」
  OK: 「他人の目とスケジュールのプレッシャーを脱ぎ捨て、"自分だけの時間"を取り戻したい——でもその方法がわからない」
...
- strategicNarrative: メインの出力。「{クリエイター名}さんの"○○"を切り口にしたコンテンツが視聴者の共感を得やすい。ブランドの課題は"△△"なので…」のような2-3文。クリエイターの動画データ・コメント傾向に基づく具体的な根拠を含めること。**「あなた」という代名詞は使わず、必ずクリエイター名（チャンネル名 + さん）で呼ぶこと。**
...
- avoidanceNote: 戦略的に合わないアプローチの警告。法的NGではなくクリエイティブ上の理由（例:「ホテルレビュー的な比較アプローチは、{クリエイター名}さんの世界観と相性が悪い」）。universalMustNotとの重複は禁止。

### インスピレーションシード（inspirationSeeds）
クリエイターの得意パターンとブランド機会の「掛け合わせの方程式」を2-3個。
- 各シード一行（30-50字）
- 「{クリエイター名}さんの"○○" × ブランドの"△△" → ？」形式
- "○○"にはクリエイターの具体的な動画タイトル・パターンを入れる
- "△△"にはブランド課題・商品特性を入れる
- 「→ ？」で終え、クリエイター自身が答えを考える余白を残す
- collabIdeasの企画タイトルそのままは禁止（あくまで「種」）

brandDescription がない場合でも campaignOverview と campaignRules は必ず生成してください。ただしブランドの詳細が不明な場合は、法的要件やNG事項を推測で生成せず、一般的なPR表記義務など確実に必要な最低限の項目のみ記載してください。
...
「安全な企画」だけでなく、「え、マジで？」と思わせる挑戦的・意外性のある企画を半数以上含めてください。
ブランドの力でスケールが上がる企画、対決・制約・視聴者参加の仕掛けがある企画を意識してください。
企画案はあくまで参考案です。クリエイターが「これ自分でもやりたい」と思えるような、クリエイターの世界観を尊重した企画にすること。
...
export async function analyzeBrandFit(
  ...
  if (!Array.isArray(raw.collabIdeas) || raw.collabIdeas.length === 0) {
    ...
    const requiredPostingFields = ["contentDirection", "descriptionBoxSuggestion", "toneAndManner", "creatorContext", "sampleOpening"] as const;
    ...
    raw.collabIdeas = raw.collabIdeas.filter((idea: unknown) => {
      ...
      if (!Array.isArray(pi.keyMessages) || !pi.keyMessages.every((m: unknown) => typeof m === "string")) return false;
      if (!Array.isArray(pi.brandMustDo) || !pi.brandMustDo.every((m: unknown) => typeof m === "string")) return false;
      if (!Array.isArray(pi.brandMustNot) || !pi.brandMustNot.every((m: unknown) => typeof m === "string")) return false;
      if (!Array.isArray(pi.creatorFreedom) || !pi.creatorFreedom.every((m: unknown) => typeof m === "string")) return false;
    ...
    collabIdeas: (raw.collabIdeas as Record<string, unknown>[]).map((idea) => {
      ...
      return {
        ...
        postingInstruction: {
          ...
          brandMustDo: (pi.brandMustDo as unknown[]).filter((m: unknown) => typeof m === "string") as string[],
          brandMustNot: (pi.brandMustNot as unknown[]).filter((m: unknown) => typeof m === "string") as string[],
          creatorFreedom: (pi.creatorFreedom as unknown[]).filter((m: unknown) => typeof m === "string") as string[],
          creatorContext: pi.creatorContext as string,
          sampleOpening: pi.sampleOpening as string,
        },
        distributionStrategy: {
